...
- **Settings → Database panel**: the prototype shows a Bootstrap API endpoint / Tenant Slug / Workstation Token form with a snapshot freshness table — that's a stylized cloud-onboarding mockup that doesn't reflect the real desktop architecture. WPF ships the actual production panel: a PostgreSQL connection-string editor with Load / Test / Save-override / Restart-API actions, masked current value, and the local override file path. See [SettingsView.xaml](../src/ShowroomBilling.Desktop/Views/Settings/SettingsView.xaml) `IsDatabaseVisible` block.
