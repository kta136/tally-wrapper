...
This document defines the target Tally Wrapper PostgreSQL schema at the level needed to implement the first production version. Production runs PostgreSQL 18, self-hosted on an Oracle Cloud Infrastructure VPS; deployment and operational responsibilities are documented in [11_deployment_and_ops.md](./11_deployment_and_ops.md#4-postgresql-deployment).
...
- new non-deterministic persisted IDs are UUIDv7 values generated by the .NET application for better B-tree insertion locality; existing UUIDv4 values and deterministic identity GUIDs remain valid
...
- GIN trigram index on `invoice_number` for the Bills tab's contains search
...
- GIN trigram index on `party_name` for the Bills tab's contains search
